...
//	memcpy((char*)&preamble,g_sBuf+PREAMBLE_OFFSET,sizeof(preamble_t));
	...
	if((pfileRear = fopen("cc2530_rear.jpg","rb")) == NULL)
	{
	...
	fread(g_sBuf+g_fileLen,1,512*1024-g_fileLen,pfileRear);

	//if ((pfileRst = fopen("C:\\Users\\FreeMan\\Desktop\\issue\\hxCompatibleModle.upfw","wb+")) == NULL)
	//{
	...
	SYSTEMTIME st;
	GetLocalTime(&st);
	//int tmStamp = st.wYear%100<<8|st.wMonth%100<<6|st.wDay%100<<4|st.wHour%100<<2|
	char nameBuf[100]={0};
	int bufIndex=0;
	memcpy(nameBuf,"hxCompatibleModle",sizeof("hxCompatibleModle")-1);
	bufIndex+=(sizeof("hxCompatibleModle")-1);
	sprintf(nameBuf+bufIndex,"%02d",(st.wYear%100));
	bufIndex+=2;
	sprintf(nameBuf+bufIndex,"%02d",(st.wMonth%100));
	bufIndex+=2;
	sprintf(nameBuf+bufIndex,"%02d",(st.wDay%100));
	bufIndex+=2;
	sprintf(nameBuf+bufIndex,"%02d",(st.wHour%100));
	bufIndex+=2;
	sprintf(nameBuf+bufIndex,"%02d",(st.wMinute%100));	
	bufIndex+=2;
	memcpy(nameBuf+bufIndex,".upfw",sizeof(".upfw")-1);
	bufIndex+=(sizeof(".upfw")-1);
	memset(nameBuf+bufIndex,0,sizeof(nameBuf)-bufIndex);


	//if ((pfileRst = fopen("hxCompatibleModle.upfw","wb+")) == NULL)
	if ((pfileRst = fopen(nameBuf,"wb+")) == NULL)
	{
		printf("open hxCompatibleModle.upfw error\r\n");
		goto Err;
	}
