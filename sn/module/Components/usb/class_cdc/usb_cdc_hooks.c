...
unsigned long Max_Lun=0;
//extern char g_modBoot;
extern void BootRfDown(void);
...
void usbcrHookProcessOut(void)
{
   ...
   }else if(usbSetupHeader.request == MASS_STORAGE_RESET)
   {
	 if(usbfwData.ep0Status == EP_IDLE)
      {
         usbSetupData.pBuffer = (uint8 __xdata *) &Max_Lun;
         usbfwData.ep0Status = EP_RX;
      }
      else if(usbfwData.ep0Status == EP_RX) { }
...
/*
   if (usbSetupHeader.request == CDC_GET_LINE_CODING) {
	...
	if (usbSetupHeader.request == GET_MAX_LUN) {
	// First the endpoint status is EP_IDLE...
	if (usbfwData.ep0Status == EP_IDLE) {
	 usbSetupData.pBuffer = (uint8 __xdata *) &Max_Lun;
