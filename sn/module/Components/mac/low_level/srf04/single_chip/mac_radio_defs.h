...
//the below is for private use
/************************************************************************************************/
#define MAC_RADIO_TRANS_NOREADY					(FSMSTAT1&(TX_ACTIVE|SFD))
#define macDisableRxInterrupt()	{RFIRQM0 &= ~BV(6); IEN2 &= ~BV(0);}	// disable RXPKTDONE interrupt
																	// disable general RF interrupts
#define macEnableRxInterrupt()	{RFIRQM0 |= BV(6);IEN2 |= BV(0);}
#define macReceiveOff()			{MAC_ISRFOFF();MAC_ISFLUSHRX();}

#define MAC_CLEAR_TX_INT()								st(RFIRQF1 = ~IRQ_TXDONE;)
#define MAC_ISTXON()									st(RFST = 0xE9;)
#define MAC_ISFLUSHRX()									st(RFST = 0xEC;)
#define MAC_ISFLUSHTX()									st(RFST = 0xEE;)

#define MAC_ISRXON()                							st(RFST = 0xE3;)
#define MAC_ISTXONCCA()             							st(RFST = 0xEA;)
#define MAC_ISRFOFF()               							st(RFST = 0xEF;)

#define AUTO_ACK                   								0x20
#define AUTO_CRC                   								0x40

#define MAC_AUTO_CRC_ACK()								st(FRMCTRL0 |= (AUTO_ACK | AUTO_CRC);)
#define MAC_RECOMMEND_SETTING()						st(TXFILTCFG = 0x09;AGCCTRL1 = 0x15;FSCAL1 = 0x00;)

#define MAC_INT_OFF()										st( EA = 0; )
#define MAC_INT_ON()										st( EA = 1; )
/************************************************************************************************/
...
#define HAL_MAC_USE_REGISTER_POWER_VALUES   //used by cbyi
