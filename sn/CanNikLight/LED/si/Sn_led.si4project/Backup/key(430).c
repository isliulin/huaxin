...
u8 g_keyLock = 0;
u8 g_shadowKey=0;
u8 g_bat1LowTwinkle = 0;
u8 g_bat2LowTwinkle = 0;
...
u8 g_pwm_chan1_duty = 0;
u8 g_pwm_chan2_duty = 0;
...
u8 ParseKey(u8 *rKey)
{
	...
	{
		if ( crtTime - g_keyTimer > 10000L)              // key timer overflow, 10ms
		{
			g_keyTimer = crtTime;
...
void KeyRightProc()
{
	...
	{/*
		...
		RC_ControlData();
#else
		CHAN_1_Set_Duty(PWN_PERIOD - (u16)g_pwm_chan1_duty*PWM_PULSE_WIDTH_STEP);
#endif
		//g_ledChar = (g_pwm_chan1_duty / PWM_PULSE_WIDTH_STEP);
		g_ledChar = g_pwm_chan1_duty;
	...
	{/*
		...
		RC_ControlData();
#else
		CHAN_2_Set_Duty(PWN_PERIOD - (u16)g_pwm_chan2_duty*PWM_PULSE_WIDTH_STEP);
#endif
		//g_ledChar = (g_pwm_chan2_duty / PWM_PULSE_WIDTH_STEP);
		g_ledChar = g_pwm_chan2_duty;
	}
...
#if(__DEVICE__==DEVICE_RC)
                case (KEY_DIR_LEFT|0x20):
			KeyLeftProc();
			delay_ms(200);
		break;

                case (KEY_DIR_RIGHT|0x20):
			KeyRightProc();
			delay_ms(200);
		break;
#endif
