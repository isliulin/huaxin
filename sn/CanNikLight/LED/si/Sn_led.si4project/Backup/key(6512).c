...
uint16_t g_pwm_chan1_duty = 2000;
uint16_t g_pwm_chan2_duty = 2000;
...
#define PWM_PULSE_WIDTH 90
//void KeyLeftProc()
void KeyRightProc()
{
	if (g_CtrlSelect)
	{
		if (g_pwm_chan1_duty > PWM_PULSE_WIDTH)
		{
			g_pwm_chan1_duty -= PWM_PULSE_WIDTH;
		}else
		{
			g_pwm_chan1_duty = 0;
		}
		CHAN_1_Set_Duty(g_pwm_chan1_duty);
		g_ledChar = (g_pwm_chan1_duty / PWM_PULSE_WIDTH)*2;
			
	}else
	{
		if (g_pwm_chan2_duty > PWM_PULSE_WIDTH)
		{
			g_pwm_chan2_duty -= PWM_PULSE_WIDTH;
		}else
		{
			g_pwm_chan2_duty = 0;
		}
		CHAN_2_Set_Duty(g_pwm_chan2_duty);
		g_ledChar = (g_pwm_chan2_duty / PWM_PULSE_WIDTH)*2;
	}
...

{
	if (g_CtrlSelect)
	{
		if (g_pwm_chan1_duty < (4500 - PWM_PULSE_WIDTH))
		{
			g_pwm_chan1_duty += PWM_PULSE_WIDTH;
		}else
		{
			g_pwm_chan1_duty = 4500;
		}
		CHAN_1_Set_Duty(g_pwm_chan1_duty);
		g_ledChar = (g_pwm_chan1_duty / PWM_PULSE_WIDTH)*2;
	...
	{
		if (g_pwm_chan2_duty < (4500 - PWM_PULSE_WIDTH))
		{
			g_pwm_chan2_duty += PWM_PULSE_WIDTH;
		}else
		{
			g_pwm_chan2_duty = 4500;
		}
		CHAN_2_Set_Duty(g_pwm_chan2_duty);
		g_ledChar = (g_pwm_chan2_duty / PWM_PULSE_WIDTH)*2;
	}
