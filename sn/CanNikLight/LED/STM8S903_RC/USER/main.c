...
void LedTest()
{
	static u16 sTestm = 0;
	static u16 cnt = 0;
	u8 testArr[] = {0x11,0x22,0x33,0x44,0x55};
	u8 readArr[5] = {0};
	...
	if (cnt > 1000)
	{
		...
		Write_EEPROM(testArr,5);
                delay_ms(10);
		Read_EEPROM(readArr,5);

#if (__UART_DBG__>0)
		printf("%x %x %x %x %x\r\n",readArr[0],readArr[1],readArr[2],readArr[3],readArr[4]);
#endif
...
void LowbatTwinkle()
{
	g_lowBatTwinkleTimer++;
	if (g_lowBatTwinkleTimer > 400)
	{
		g_lowBatTwinkleTimer = 0;
		if (g_bat1LowTwinkle)
		{
			Led_1_twinkle();
		}
...
void LED_init()
{
	...
	{
		CHAN_1_Set_Duty(PWN_PERIOD);
		CHAN_2_Set_Duty(PWN_PERIOD);
	}
}
