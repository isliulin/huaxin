...
void A7105_Init(void)
{
	...
	A7105_Write_Reg(0x03,RF_PACKET_LEN-1);
	//reg = A7105_Read_Reg(0x03);
	...
	while ((0x01 & A7105_Read_Reg(0x02)))
	{
		delay_ms(10);
	}
	...
	A7105_Read_Ram(0x06, g_rfIdBuf,4);
	...
	A7105_strobe_cmd(0xF0);   // FIFO READ RESET
	A7105_strobe_cmd(0xC0);   // FIFO RX MODE
#elif (__DEVICE__==DEVICE_RC)
	A7105_strobe_cmd(0xE0);   // FIFO READ RESET
	A7105_strobe_cmd(0xD0);   // FIFO RX MODE
#endif
...
void ParseRfData(u8 *dat)
{
		...
		if ( 0 == dat[0] )
		{
			g_pwm_chan1_duty = 0;
			g_pwm_chan2_duty = 0;
			CHAN_1_Set_Duty(PWN_PERIOD - g_pwm_chan1_duty*PWM_PULSE_WIDTH_STEP);
			CHAN_2_Set_Duty(PWN_PERIOD - g_pwm_chan2_duty*PWM_PULSE_WIDTH_STEP);
			g_ledChar = 0;
			g_isLedON = 0;
			// 关掉所有显示
		...
		{
			//g_pwm_chan1_duty = dat[1] * PWM_PULSE_WIDTH_STEP;
			//g_pwm_chan2_duty = dat[2] * PWM_PULSE_WIDTH_STEP;
			g_pwm_chan1_duty = dat[1];
			g_pwm_chan2_duty = dat[2];
			CHAN_1_Set_Duty(PWN_PERIOD - g_pwm_chan1_duty*PWM_PULSE_WIDTH_STEP);
			CHAN_2_Set_Duty(PWN_PERIOD - g_pwm_chan2_duty*PWM_PULSE_WIDTH_STEP);
			g_CtrlSelect = dat[3];
			if (g_CtrlSelect)
...
void A7105_Slave_Proc(void)
{
	u8 rfBuf[RF_PACKET_LEN];      // 命令格式，[55][AA][LEN][D1][D2][D..][DN][CRC]
	u8 reg;
#if (__UART_DBG__ > 0)
	u8 i;		
#endif

	reg = (GPIO_ReadInputPin(GPIOC,GPIO_PIN_3) >> 3) & 0x01;
	
	if (reg == RESET)    // 数据
	{/*
		if (((GPIO_ReadInputPin(GPIOA,GPIO_PIN_3) >> 3) & 0x01) == RESET)
		{
		...
		A7105_Read_Ram(0x05,rfBuf,RF_PACKET_LEN);
...
void RC_ControlData(void)
{
	...
	g_rfSendBuf[0] = 0x55;
	g_rfSendBuf[1] = 0xAA;
	g_rfSendBuf[2] = RF_PACKET_LEN - 2;
	g_rfSendBuf[3] = g_isLedON;       //
	//g_rfSendBuf[4] = (g_pwm_chan1_duty/PWM_PULSE_WIDTH_STEP);
	//g_rfSendBuf[5] = (g_pwm_chan2_duty/PWM_PULSE_WIDTH_STEP);
	g_rfSendBuf[4] = g_pwm_chan1_duty;
	g_rfSendBuf[5] = g_pwm_chan2_duty;
	g_rfSendBuf[6] = g_CtrlSelect;
	g_rfSendBuf[7] = 0;
	for (i=2;i<7;i++)
	{
		g_rfSendBuf[7] += g_rfSendBuf[i]; 
	}
	g_rfSendBuf[47] = 0x55;
	g_rfSendBuf[48] = 0xAA;
	...
	//A7105_Write_Ram(0x05,g_rfSendBuf,RF_PACKET_LEN);	
	
}

void RfSendRcvTest(void)
{

#if (__DEVICE__ == DEVICE_RC)

	g_rfTestTimer++;
	if (g_rfTestTimer > 2000)
	{
		g_rfTestTimer = 0;
		//A7105_strobe_cmd(0xE0);   // FIFO Send RESET
		//A7105_strobe_cmd(0xD0);   // FIFO RX MODE
		RC_ControlData();
	}
