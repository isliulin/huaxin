...
typedef struct U_STCOMMANDER
{
	...
	unsigned char cmd_buf[RCV_CMD_LEN];
	unsigned char verify;
	unsigned char sum;
}STCOMMANDER;

STCOMMANDER stCommander;
//extern char g_downLoad_enable;
//extern char g_downLoad_error; 
//extern char g_erase_flag;
//extern char g_JumpRet;
//extern char g_SecProtectRet;
//extern char g_VerifyRet;
//extern char g_secProtectErr;
//extern char g_blockLast;
//extern char g_UartShutRet;
//
// Constructor
//
...
// Write a character.
//
void CSerialPort::ComfirmProc(unsigned char* rcv)
{
	unsigned char *str;
//	char *str1;
//    int k, nEvent;//, i;
...
//	static unsigned char CfmBuf[80];
	...
	if (!loader)
	{
		if (*str==0x7e) {
			loader = 1;
			rcvIndex = 0;
			memset(stCommander.cmd_buf,0,sizeof(stCommander.cmd_buf));
			stCommander.sum = 0;
	...
	}else
	{
		...
		{	
			...
			{
				stCommander.cmd_buf[bufIndex++] = *str;
				stCommander.sum += *str;
			}else
			{
				//stCommander.cmd_buf[bufIndex++] = *str;				//sum
				loader = 0;
				stCommander.verify = *str;
				if (stCommander.sum != stCommander.verify)
				{
					//g_downLoad_enable = 0;
					loader = 0;
					rcvIndex = 0;
...
//						g_downLoad_enable = 0;
					...
					{
//						g_downLoad_enable = -1;
//						g_downLoad_error = stCommander.cmd_buf[0];
						rcvIndex = 0;
						stCommander.sum = 0;
					}
					break;
					...
					if (stCommander.cmd_buf[1]!=0) 
					{
						if (0xee==stCommander.cmd_buf[0]) 
						{
//							g_erase_flag = -1;							
						}
					...
					{
						if (0x55==stCommander.cmd_buf[0]) 
						{
//							g_erase_flag = 1;
						}else if (0xaa==stCommander.cmd_buf[0])
						{
//							g_erase_flag = 2;
						}
					}
