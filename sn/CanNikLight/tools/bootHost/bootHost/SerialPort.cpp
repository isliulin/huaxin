						   ...
						   UINT  writebuffersize)	// size to the writebuffer
{
	...
	//strSzPort.Format(_T("COM%d"), portnr);
	if(portnr<10)
	{
		strSzPort.Format(_T("COM%d"), portnr);
	}else
	...
	strBaud.Format(_T("baud=%d parity=%c data=%d stop=%d"), baud, parity, databits, stopbits);

	// get a handle to the port
	m_hComm = CreateFile(strSzPort/*(LPCWSTR)szPort*/,						// communication port string (COMX)
