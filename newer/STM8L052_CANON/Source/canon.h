...
#define CANON_SCK_GPIO          GPIOB
#define CANON_SCK_PIN           GPIO_Pin_5
#define IGBT_DRIVER_GPIO        GPIOE
#define IGBT_DRIVER_PIN         GPIO_Pin_7


#ifdef SN_TEST
#define SPI_MISO_AUX_HIGH       GPIOF->ODR |= GPIO_Pin_1;
#define SPI_MISO_AUX_LOW        GPIOF->ODR &= (uint8_t)(~GPIO_Pin_1);
#define SPI_MISO_AUX(x)         {if(x){GPIOF->ODR |=GPIO_Pin_1;}else{GPIOF->ODR &= (uint8_t)(~GPIO_Pin_1);}}
#define COM_AUX_HIGH            GPIOF->ODR |= GPIO_Pin_0
#define COM_AUX_LOW             GPIOF->ODR &= (uint8_t)(~GPIO_Pin_0)
#else
...
#define SCK_IN                 CANON_SCK_GPIO->IDR & (uint8_t)CANON_SCK_PIN

#define IGBT_DRIVER_Start      GPIO_SetBits(IGBT_DRIVER_GPIO, IGBT_DRIVER_PIN)
#define IGBT_DRIVER_Stop       GPIO_ResetBits(IGBT_DRIVER_GPIO, IGBT_DRIVER_PIN)

#define Enable_Flash_IN_Int    GPIOD->CR2 |= GPIO_Pin_4;              
#define Disable_Flash_IN_Int   GPIOD->CR2 &= (uint8_t)(~(GPIO_Pin_4));  

#define Enable_SCK_IN_Int      GPIOB->CR2 |= GPIO_Pin_5;              
#define Disable_SCK_IN_Int     GPIOB->CR2 &= (uint8_t)(~(GPIO_Pin_5));  
...
#define DisableSckInt() {GPIOB->CR2 &= (uint8_t)(~(CANON_SCK_PIN));}    //关SCK中断}
#define EnableSckInt()  {GPIOB->CR2 |= CANON_SCK_PIN;}                  //开SCK中断
#define ShutSpiClock()  {CLK->PCKENR1 &=  ~( 1 << 4 );}
#define OpenSpiClock()  {CLK->PCKENR1 |=   ( 1 << 4 );}
...
typedef struct MOTO_SET
{
	...
	u8 *mode;               //mode manu or auto 马达模式，0为auto， 1为manu
...
typedef struct FLASHPARAMETER
{
	...
	s8 charge_ok;		//recored the charge statue	        是否充电完成 0 充电中 1：充电可闪 2：充电到可发送指令
	s8 statue_link;	        //recored whether linking the camera    是否连接相机
...
//extern _Flash_Time_TypeDef  Flash_Time_Group[6][28];
