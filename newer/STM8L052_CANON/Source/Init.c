...
uint16_t ADC_Buffer[BUFFER_COUNT][BUFFER_SIZE]  = {0};
...
void GPIO_Config(void)
{
    ...
    GPIO_Init(GPIOB, GPIO_Pin_2, GPIO_Mode_Out_PP_High_Fast);    //READY
    ...
    GPIO_Init(GPIOD, GPIO_Pin_4, GPIO_Mode_In_PU_No_IT);           //FLASH_IN
    ...
    GPIO_Init(GPIOC, GPIO_Pin_1, GPIO_Mode_In_PU_No_IT);           //PHOTOSEN
...
#else
    //CPU->CCR=0X28;
    GPIO_Init(GPIOF, GPIO_Pin_1, GPIO_Mode_Out_PP_High_Fast);
    GPIO_Init(GPIOF, GPIO_Pin_0, GPIO_Mode_Out_PP_High_Fast);   
    GPIO_Init(GPIOF, GPIO_Pin_6, GPIO_Mode_Out_PP_Low_Fast);   // dc5v
    GPIO_Init(GPIOF, GPIO_Pin_5, GPIO_Mode_Out_PP_Low_Fast);   // 339
    
    GPIO_Init(GPIOC, GPIO_Pin_6, GPIO_Mode_Out_PP_Low_Fast);   // LED1
    GPIO_Init(GPIOC, GPIO_Pin_5, GPIO_Mode_Out_PP_Low_Fast);   // LED2
    
    GPIO_WriteBit(GPIOF,GPIO_Pin_6,SET);
    GPIO_WriteBit(GPIOF,GPIO_Pin_5,SET);
    
    GPIO_WriteBit(GPIOC,GPIO_Pin_6,SET);
    //GPIO_WriteBit(GPIOC,GPIO_Pin_5,SET);    
#endif
    
    ITC_SetSoftwarePriority(EXTI1_IRQn, ITC_PriorityLevel_3);//等级3
    EXTI_SetPinSensitivity(EXTI_Pin_1, EXTI_Trigger_Falling);//下降沿触发
}
...
void ADC_Config(void)
{
    ...
    ADC_SamplingTimeConfig(ADC1, ADC_Group_SlowChannels, ADC_SamplingTime_384Cycles); //AN  0-23
    //ADC_SamplingTimeConfig(ADC1, ADC_Group_FastChannels, ADC_SamplingTime_384Cycles); //AN 24-27
    ...
    ADC_DMACmd(ADC1, ENABLE);							//先开DMA，再开ADC，避免数据错位
    ADC_Cmd(ADC1, ENABLE);
...
void DMA_Config(void)
{
    ...
    DMA_ITConfig(DMA1_Channel0, DMA_ITx_TC, ENABLE);                            //DMA1 channel0 Transfer complete interrupt    
