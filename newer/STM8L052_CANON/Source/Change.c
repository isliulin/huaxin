...
{
    ADC_VALUE.Channel_CAP_VALUE  = Average_math_filter(ADC_CAP_AN,10);
    ADC_VALUE.Channel_KEY_VALUE  = Average_math_filter(ADC_KEY_AN,5);
    ADC_VALUE.Channel_LAMP_VALUE = Average_math_filter(ADC_LAMP_AN,5);
    ADC_VALUE.Channel_AF_VALUE   = Average_math_filter(ADC_AFIN_AN,5);
    ADC_VALUE.Channel_MOS_VALUE  = Average_math_filter(ADC_MOS_AN,5);
}
#define N  11 
/*************************************************************************************
* FunctionName   : Average_math_filter()
* Description    : ADC滤波
* EntryParameter : 均值法   times 要小于 N
* ReturnValue    : None
**************************************************************************************/
...
/*************************************************************************************
* FunctionName   : Bubble_method_filter()
* Description    : ADC滤波
* EntryParameter : 冒泡法 Bubble method
* ReturnValue    : None
**************************************************************************************/
...
void Change_Cap(void)           //10ms调用一次
{
    ...
    {
        if(CAP_VALUE > CAP_330V_VALUE)
        {
            CHANGE_CAP_STOP;
        }
        else if(CAP_VALUE < CAP_330V_VALUE-100)
        {
            if(CAP_VALUE < CAP_170V_VALUE)
            {
                GPIO_H_CHANGE_DISABLE;
            }
            else if(CAP_VALUE > CAP_170V_VALUE+20)
            {
