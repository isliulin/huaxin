...
void DMA_Callback(void)
{
    ADC_VALUE.Channel_CAP_VALUE  = Average_math_filter(4,10);
    ADC_VALUE.Channel_KEY_VALUE  = Average_math_filter(0,5);
    ADC_VALUE.Channel_LAMP_VALUE = Average_math_filter(1,5);
    ADC_VALUE.Channel_AF_VALUE   = Average_math_filter(2,5);
    ADC_VALUE.Channel_MOS_VALUE  = Average_math_filter(3,5);
}
...
{
    uint8_t count;
    uint16_t adc_buf_temp = 0;
    for ( count=0;count<times;count++)  
    { 
        adc_buf_temp += ADC_Buffer[count][channel];  
    }
    return (uint16_t)(adc_buf_temp/times);
}
...
uint16_t Bubble_method_filter(uint8_t channel)  
{  
    uint16_t value_buf1[11] = {0}; 
    uint8_t count,i,j;
    uint16_t temp;   
    
    for ( count=0;count<11;count++)  
    { 
        value_buf1[count] = ADC_Buffer[count][channel];  
    }
...
void Change_Cap(void)           
{
    ...
    if((Motor_Manger_TypeDef.Motor_Move_State == Motor_Move_End) &        (SysInfo.Change_Cap_En_flag == 1)&        (SysInfo.Low_Battery_flag == 0))
    {
        if(CAP_VALUE > 3383)
        {
            {TIM5_CtrlPWMOutputs(DISABLE);GPIO_ResetBits(((GPIO_TypeDef *) (uint16_t)0x500A), GPIO_Pin_6);};
        }
        else if(CAP_VALUE < 3383-100)
        {
            if(CAP_VALUE < 1743)
            {
                GPIO_ResetBits(((GPIO_TypeDef *) (uint16_t)0x500A), GPIO_Pin_3);
            }
            else if(CAP_VALUE > 1743+20)
            {
                GPIO_SetBits(((GPIO_TypeDef *) (uint16_t)0x500A), GPIO_Pin_3);
            }
            {TIM5_CtrlPWMOutputs(ENABLE); GPIO_SetBits(((GPIO_TypeDef *) (uint16_t)0x500A), GPIO_Pin_6);};
        }        
    }
    else
    {
...
void Low_Batt_Check(void)
{
    ...
    if(Cap_Value > 2820)                          
    {
        {GPIO_ResetBits(((GPIO_TypeDef *) (uint16_t)0x500F),GPIO_Pin_6);GPIO_SetBits(((GPIO_TypeDef *) (uint16_t)0x500F),GPIO_Pin_7);};
        SysInfo.Cap_Full_Flag = 1;                          

        
        if(BEEP_CAP_ONCE)
        {
            BEEP_CAP_ONCE = 0;
            BeepFlag.Beep_CapFull_Flag = 1;                  
        }
    }
    else if(Cap_Value < (2820-100))
    {        
        {GPIO_SetBits(((GPIO_TypeDef *) (uint16_t)0x500F),GPIO_Pin_6);GPIO_ResetBits(((GPIO_TypeDef *) (uint16_t)0x500F),GPIO_Pin_7);};
        SysInfo.Cap_Full_Flag = 0;

        BEEP_CAP_ONCE = 1;
    }        
    
    if((Cap_Value < 2820) &         (SysInfo.Change_Cap_En_flag)&         (Motor_Manger_TypeDef.Motor_Move_State == Motor_Move_End))
    {
...
void Check_Lamp_Temp(void)
{
    ...
    if(Lamp_Value > 3528)
    {
        ...
        {
            BEEP_TEMP_ONCE = 0;
            BeepFlag.Beep_LampTemp_Flag  = 1;  
        }        
    }
    else if(Lamp_Value > 3300)
    {
