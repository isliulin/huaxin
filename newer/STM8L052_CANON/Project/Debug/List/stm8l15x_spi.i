...
void SPI_DeInit(SPI_TypeDef* SPIx)
{
  SPIx->CR1    = ((uint8_t)0x00);
  SPIx->CR2    = ((uint8_t)0x00);
  SPIx->CR3    = ((uint8_t)0x00);
  SPIx->SR     = ((uint8_t)0x02);
  SPIx->CRCPR  = ((uint8_t)0x07);
}
...
void SPI_Cmd(SPI_TypeDef* SPIx, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    SPIx->CR1 |= ((uint8_t)0x40);  
  }
  else
  {
    SPIx->CR1 &= (uint8_t)(~((uint8_t)0x40));  
  }
}
...
void SPI_BiDirectionalLineConfig(SPI_TypeDef* SPIx, SPI_Direction_TypeDef SPI_Direction)
{
  ...
  {
    SPIx->CR2 |= ((uint8_t)0x40);  
  }
  else
  {
    SPIx->CR2 &= (uint8_t)(~((uint8_t)0x40));  
  }
}
...
void SPI_CalculateCRCCmd(SPI_TypeDef* SPIx, FunctionalState NewState)
{
  ...
  SPI_Cmd(((SPI_TypeDef *) (uint16_t)0x5200), DISABLE);

  if (NewState != DISABLE)
  {
    SPIx->CR2 |= ((uint8_t)0x20);  
  }
  else
  {
    SPIx->CR2 &= (uint8_t)(~((uint8_t)0x20));  
  }
}
