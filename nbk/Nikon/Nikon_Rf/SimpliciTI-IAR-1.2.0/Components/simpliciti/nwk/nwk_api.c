...
static void MstPacketParse(void);

static uint8 SwMode(uint8 sc);

extern uint16 HalUARTWrite(uint8 port, uint8 *buf, uint16 len);

extern void UartDMAIntSend(void);

static void FlashInfoSend(void);

static void PreFlashInfoSend(void);

extern void macLowLevelInit(void);

extern void macRadioSetTxPower(uint8 txPower);

extern void macSleepWakeUp(void);

/******************************************************************************
 * GLOBAL VARIABLES
 */
frameInfo_t g_frameInfo;
extern uint8_t g_rcvPkt;
mrfiPacket_t * g_pRcvPkt;
RadioSet radioSet={0};
extern unsigned char g_veriBuf2[];
...
smplStatus_t SMPL_Init(uint8_t (*f)(linkID_t))
{
  ...
  if (!sInit_done)
  {
    ...
    MRFI_DisableRxAddrFilter();                       //2016/07/11 cbyi
#endif
  }
  sInit_done = 1;
...
static void FlashInfoSend(void)
{
	...
	radioSet.shutter =0x00;
	radioSet.apert = 0x00;
	radioSet.isoCmp = 0x00;

	bufIndex = 0;
	devSBuf[bufIndex++] = 0xAA;
	devSBuf[bufIndex++] = 0x00;
	devSBuf[bufIndex++] = UART_SLAVE_FLA;

	devSBuf[bufIndex++] = radioSet.apert;
	devSBuf[bufIndex++] = radioSet.shutter;
	devSBuf[bufIndex++] = radioSet.isoCmp;

	devSBuf[bufIndex++] = SwMode(g_pRcvPkt->frame[28]>>4);//mode
	devSBuf[bufIndex++]= g_pRcvPkt->frame[31];

	devSBuf[bufIndex++] = SwMode(g_pRcvPkt->frame[28]&0x0f);//mode
	devSBuf[bufIndex++]= g_pRcvPkt->frame[32];

	devSBuf[bufIndex++] = SwMode(g_pRcvPkt->frame[29]>>4);//mode
	devSBuf[bufIndex++]= g_pRcvPkt->frame[33];

	devSBuf[bufIndex++] = SwMode(g_pRcvPkt->frame[29]&0x0f);//mode
	devSBuf[bufIndex++]= g_pRcvPkt->frame[34];

	devSBuf[bufIndex++] = SwMode(g_pRcvPkt->frame[30]>>4);//mode
	devSBuf[bufIndex++]= g_pRcvPkt->frame[35];

	devSBuf[bufIndex++] = SwMode(g_pRcvPkt->frame[30]&0x0f);//mode
	devSBuf[bufIndex++]= g_pRcvPkt->frame[36];
...
static void RptFlashInfoSend(void)
{
	uint8 bufIndex=0;
	uint8 devSBuf[20];
	uint8 i=0;
	...
	for(i=0;i<5;i++)
	{
		...
		devSBuf[bufIndex++] = g_pRcvPkt->frame[29];		
	}

	devSBuf[bufIndex++] = g_pRcvPkt->frame[30];                 // hz
	devSBuf[bufIndex++] = g_pRcvPkt->frame[31];                 // times
...
uint8 g_flashReady = 0;
uint8 g_preFlashRdy = 0;
uint8 g_rptFlashRdy = 0;
uint16 g_flashSignal = 0;
static void MstPacketParse(void)
{
	if((0x24!=g_pRcvPkt->frame[2])||(0x21!=g_pRcvPkt->frame[3]))
	{
		return;
	}

	if((0x1E==g_pRcvPkt->frame[0])||((0x21==g_pRcvPkt->frame[0])&&(0x01==g_pRcvPkt->frame[17])))
	{
#if 1
		MRFI_DelayMs(5);
		memcpy(g_frameInfo.mrfiPkt.frame,g_veriBuf,43);
  		g_frameInfo.fi_usage = FI_AVAILABLE;
		nwk_sendFrame(&g_frameInfo,MRFI_TX_TYPE_FORCED);
		LED_INS_LINKED();
#endif
	}

	if((0x21==g_pRcvPkt->frame[0])&&(0xB2==g_pRcvPkt->frame[17]))
	{
		if(!g_rptFlashRdy)
		{
			RptFlashInfoSend();
			g_rptFlashRdy = 1;
		}else if(0x47 == g_pRcvPkt->frame[25])
		{
			MRFI_DelayMs(6);
			UART_INT_SET();
			g_rptFlashRdy = 0;
	...
	if(0x1D==g_pRcvPkt->frame[0])
	{
		...
		if(!g_preFlashRdy)
		{
			PreFlashInfoSend();
			g_preFlashRdy = 1;
		}else if(0x08 == g_pRcvPkt->frame[21])
		{
	...
	if(0x18==g_pRcvPkt->frame[0])
	{
		if(0x25==g_pRcvPkt->frame[14])
		{
			//UART_INT_TOGGLE();
#if 0
			if(1==g_flashReady)
			{
				g_flashReady = 2;
				g_flashSignal = MAKEWORD(g_pRcvPkt->frame[20]+2, 0x09);

			}else if(2==g_flashReady)
			{
				//tx_send(&g_pRcvPkt->frame[20],2);
				if(g_flashSignal == MAKEWORD(g_pRcvPkt->frame[20], g_pRcvPkt->frame[21]))
				{
					//tx_send("fla\r\n",sizeof("fla\r\n"));
					UART_INT_SET();
					g_flashSignal = 0;
...
#endif
		}else if(0x32==g_pRcvPkt->frame[14])
		{
			MRFI_DelayMs(20);
			memcpy(g_frameInfo.mrfiPkt.frame,g_veriBuf2,41);
	  		g_frameInfo.fi_usage = FI_AVAILABLE;
			nwk_sendFrame(&g_frameInfo,MRFI_TX_TYPE_FORCED);
		}
...
void NIK_LINK(void)
{
	...
	{
		g_rcvPkt = 0;
		//tx_send_wait(&g_pRcvPkt->frame[35], 8);
              //MRFI_DelayMs(20);
              //tx_send("rcv...\r\n",sizeof("rcv...\r\n"));
		MstPacketParse();
	}
...
#if 0
	//memcpy(g_frameInfo.mrfiPkt.frame,g_veriBuf,43);
	memcpy(g_frameInfo.mrfiPkt.frame,g_mstBuf,34);
	g_frameInfo.fi_usage = FI_AVAILABLE;
	nwk_sendFrame(&g_frameInfo,MRFI_TX_TYPE_FORCED);
	MRFI_DelayMs(800);
#endif
        
	//Mrfi_DelayUsecLong(800,0,NULL);	
	//MRFI_DelayMs(800);
}
