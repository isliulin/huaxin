 ...
 *   - - - - - - - - - - - - - - - - - -
 */
#define MRFI_DSN_OFFSET             1//__mrfi_DSN_OFS__
...
#define MFRI_MDMCTRL0_DFLT  0x85
#define MRFI_FRMCTRL0_DFLT  0x40
#define MRFI_PREAMBLE_SIZE  ( ( ( MFRI_MDMCTRL0_DFLT >> 1 ) & 0xF ) + 2 )
#define MRFI_CRC_SIZE       ( ( MRFI_FRMCTRL0_DFLT >> 5 ) & 2 )
...
void MRFI_Receive(mrfiPacket_t * pPacket)
{
  ...
  //CD 24 21 32 02 01 77 26 00 2C 25 2C 50 10 00 00 02 00 05
...
void MRFI_SetLogicalChannel(uint8_t chan)
{
   ...
   * control register
   */
  //FREQCTRL = FREQ_2405MHZ + phyChannel;    // non 802.15.4 channels for FHSS code
  FREQCTRL  = FREQ_2405MHZ + (5 * (phyChannel - 11));
#endif
