...
#if !defined(SW_TIMER)
{
#define BSP_TIMER_TICK_FREQ   ((BSP_CLOCK_MHZ)/4)   /* Dividing by 4 see T3CTL configuration below. */
  T3CTL = (0x2 << 5) | BV(2); /* (tick freq)/4 and clear the count */
  T3CCTL0 = BV(2);            /* Compare Enable, interrupt disabled. */
  T3CC0 = ((uint8_t)(usec)) * ((uint8_t)(BSP_TIMER_TICK_FREQ));
  T3CTL |= BV(4);              /* Start cunting */
  while(!(TIMIF & BV(1)));     /* Wait */
  T3CTL = 0x00;                /* Stop counting. */
  TIMIF &= ~BV(1);             /* Clear the interrupt flag */
