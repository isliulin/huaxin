/**************************************************************************************************
  ...
  YOU FURTHER ACKNOWLEDGE AND AGREE THAT THE SOFTWARE AND DOCUMENTATION ARE PROVIDED ï¿½AS ISï¿?  WITHOUT WARRANTY OF ANY KIND, EITHER EXPRESS OR IMPLIED, INCLUDING WITHOUT LIMITATION, ANY
...
*/
#define __mrfi_GDO0_BIT__                     0
#define MRFI_GDO0_BASE (MOD2_CONNECTION ? GPIO_PORTG_BASE : GPIO_PORTB_BASE)
#define MRFI_CONFIG_GDO0_PIN_AS_INPUT()       st( MAP_GPIOPinTypeGPIOInput(MRFI_GDO0_BASE, BV(__mrfi_GDO0_BIT__)); )
#define MRFI_GDO0_PIN_IS_HIGH()               (MAP_GPIOPinRead(MRFI_GDO0_BASE, BV(__mrfi_GDO0_BIT__)))

#define MRFI_GDO0_INT_VECTOR                  (MOD2_CONNECTION ? INT_GPIOG : INT_GPIOB)
#define MRFI_ENABLE_GDO0_INT()                st( MAP_GPIOPinIntEnable(MRFI_GDO0_BASE, BV(__mrfi_GDO0_BIT__)); )
#define MRFI_DISABLE_GDO0_INT()               st( MAP_GPIOPinIntDisable(MRFI_GDO0_BASE, BV(__mrfi_GDO0_BIT__)); )
#define MRFI_GDO0_INT_IS_ENABLED()              ( HWREG(MRFI_GDO0_BASE + GPIO_O_IM) & BV(__mrfi_GDO0_BIT__) )
#define MRFI_CLEAR_GDO0_INT_FLAG()            st( MAP_GPIOPinIntClear(MRFI_GDO0_BASE, BV(__mrfi_GDO0_BIT__)); )
...
*/
#define __mrfi_GDO2_BIT__                     1
#define MRFI_GDO2_BASE (MOD2_CONNECTION ? GPIO_PORTG_BASE : GPIO_PORTB_BASE)
#define MRFI_CONFIG_GDO2_PIN_AS_INPUT()       st( MAP_GPIOPinTypeGPIOInput(MRFI_GDO2_BASE, BV(__mrfi_GDO2_BIT__)); )
#define MRFI_GDO2_PIN_IS_HIGH()               (MAP_GPIOPinRead(MRFI_GDO2_BASE, BV(__mrfi_GDO2_BIT__)))

#define MRFI_GDO2_INT_VECTOR                  (MOD2_CONNECTION ? INT_GPIOG : INT_GPIOB)
#define MRFI_ENABLE_GDO2_INT()                st( MAP_GPIOPinIntEnable(MRFI_GDO2_BASE, BV(__mrfi_GDO2_BIT__)); )
#define MRFI_DISABLE_GDO2_INT()               st( MAP_GPIOPinIntDisable(MRFI_GDO2_BASE, BV(__mrfi_GDO2_BIT__)); )
#define MRFI_GDO2_INT_IS_ENABLED()             ( HWREG(MRFI_GDO2_BASE + GPIO_O_IM) & BV(__mrfi_GDO2_BIT__) )
#define MRFI_CLEAR_GDO2_INT_FLAG()            st( MAP_GPIOPinIntClear(MRFI_GDO2_BASE, BV(__mrfi_GDO2_BIT__)); )
...
#define MRFI_VREG_EN_BASE (MOD2_CONNECTION ? GPIO_PORTG_BASE : GPIO_PORTB_BASE)
 ...
 * ------------------------------------------------------------------------------------------------
 */
#define MRFI_RESETN_GPIO_BIT                 (MOD2_CONNECTION ? 5 : 3)
#define MRFI_RESETN_BASE (MOD2_CONNECTION ? GPIO_PORTH_BASE : GPIO_PORTE_BASE)
...
*/
#define __mrfi_TX_FRM_DONE_BASE  GPIO_PORTG_BASE
#define __mrfi_TX_FRM_DONE_BIT   5
#define __mrfi_FIFO_BASE         GPIO_PORTA_BASE
#define __mrfi_FIFO_BIT          6
#define __mrfi_FIFOP_BASE        GPIO_PORTA_BASE
#define __mrfi_FIFOP_BIT         7

#define MRFI_FIFOP_INT_VECTOR    INT_GPIOA
/* Port status */
...
#define MOD1_SPI_CSN_BASE                      GPIO_PORTA_BASE
#define MOD1_SPI_CSN_BIT                       3
//#define MOD2_SPI_CSN_BASE                      GPIO_PORTJ_BASE
#define MOD2_SPI_CSN_BASE                      GPIO_PORTH_BASE
...
/* SCLK Pin Configuration (PA2) */
#define __mrfi_SPI_SCLK_GPIO_BIT__            2
#define MRFI_SPI_CONFIG_SCLK_PIN_AS_OUTPUT()  st( MAP_GPIOPinTypeSSI(GPIO_PORTA_BASE, BV(__mrfi_SPI_SCLK_GPIO_BIT__)); )
#define MRFI_SPI_DRIVE_SCLK_HIGH()            /* Not used. */
#define MRFI_SPI_DRIVE_SCLK_LOW()             /* Not used. */

/* SI Pin Configuration (PF5) */
/* SI Pin Configuration (PA5) */
#define __mrfi_SPI_SI_GPIO_BIT__              5
#define MRFI_SPI_CONFIG_SI_PIN_AS_OUTPUT()    st( MAP_GPIOPinTypeSSI(GPIO_PORTA_BASE, BV(__mrfi_SPI_SI_GPIO_BIT__)); )
#define MRFI_SPI_DRIVE_SI_HIGH()              /* Not used. */
#define MRFI_SPI_DRIVE_SI_LOW()               /* Not used. */

/* SO Pin Configuration (PF4) */
/* SO Pin Configuration (PA4) */
#define __mrfi_SPI_SO_GPIO_BIT__              4
#define MRFI_SPI_CONFIG_SO_PIN_AS_INPUT()     st( MAP_GPIOPinTypeSSI(GPIO_PORTA_BASE, BV(__mrfi_SPI_SO_GPIO_BIT__)); )
#define MRFI_SPI_SO_IS_HIGH()                 (MAP_GPIOPinRead(GPIO_PORTA_BASE, BV(__mrfi_SPI_SO_GPIO_BIT__)))

/* SPI Port Configuration */
#define MRFI_SPI_CONFIG_PORT()                /* Not used. */

/* read/write macros */
#define MRFI_SPI_WRITE_BYTE(x)                st( MAP_SSIDataPut(SSI0_BASE, (x));                       \
                                                  {                                                     \
...
#define MRFI_SPI_WAIT_DONE()                  while(MAP_SSIBusy(SSI0_BASE));
...
st (                                                                          \
  ...
  MAP_SysCtlPeripheralEnable((MOD2_CONNECTION ?                               \
                              SYSCTL_PERIPH_GPIOG: SYSCTL_PERIPH_GPIOA));     \
  MAP_SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOE);                            \
  MAP_SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOG);                            \
  MAP_SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOA);                            \
  MAP_GPIOPinConfigure(GPIO_PA4_SSI0CLK);                                     \
  MAP_GPIOPinConfigure(GPIO_PA5_SSI0TX);                                      \
  MAP_GPIOPinConfigure(GPIO_PA4_SSI0RX);                                      \
  MAP_SSIConfigSetExpClk(SSI0_BASE, MAP_SysCtlClockGet(),                     \
                         SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, SPI_RATE, 8);  \
...
#define MRFI_SPI_IS_INITIALIZED()  (HWREG(SSI0_BASE + SSI_O_CR1) & SSI_CR1_SSE)

/**************************************************************************************************
 *                                  Compile Time Integrity Checks
 **************************************************************************************************
 */
#ifndef BSP_BOARD_DK_LM3S9B96
