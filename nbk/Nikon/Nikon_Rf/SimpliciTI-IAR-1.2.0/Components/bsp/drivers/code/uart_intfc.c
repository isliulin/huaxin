...
//serial setting
#define UART_DEFAULT_BAUDRATE			HAL_UART_BR_115200//HAL_UART_BR_57600
#define UART_DEFAULT_OVERFLOW			FALSE
#define UART_DEFAULT_THRESHOLD		(128/2)
#define UART_DEFAULT_MAX_RX_BUFF		128		
#define UART_DEFAULT_MAX_TX_BUFF		128
#define UART_DEFAULT_IDLE_TIMEOUT		6
...
void DmaUartInit (void)
{
	...
	uartConfig.baudRate             = UART_DEFAULT_BAUDRATE;
	uartConfig.flowControl          = UART_DEFAULT_OVERFLOW;
	uartConfig.flowControlThreshold = UART_DEFAULT_THRESHOLD;
	uartConfig.rx.maxBufSize        = UART_DEFAULT_MAX_RX_BUFF;
	uartConfig.tx.maxBufSize        = UART_DEFAULT_MAX_TX_BUFF;
	uartConfig.idleTimeout          = UART_DEFAULT_IDLE_TIMEOUT;
...
static void UartRcvHandle(uint8 *rcv,uint8 len)
{
	//HalUARTWrite(0,"Receive data\r\n",sizeof("Receive data\r\n"));
	//UartDMAIntSend();
	//HalUARTWrite(0,rcv,len);
	//UartDMAIntSend();
	//HalUARTPoll();
	//UartDMAIntSend();
	return;
...
static void UartProcessData(uint8 port, uint8 event)
{
	...
	rcvLen = Hal_UART_RxBufLen(port);
	osal_memset(rx_buff, 0x00, RX_TX_BUFFER_SIZE);
	HalUARTRead (port, rx_buff, rcvLen);

	UartRcvHandle(&rx_buff[0],rcvLen);
}
