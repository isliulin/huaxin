...
//#ifdef NBK600EX_DEBUG
			...
			||defined(DSP_TEST)||defined(RGB_TEST)||defined(SLAVE_TEST))
	uart1_init(72,115200);
...
void LightSlaveFlash(void)
{
	switch(wlSlave.s_wlMode)
	{
		...
		case WORKINGMODE_MULTI:
			...
			FlashInResMode(WORKINGMODE_MULTI,canWireless.wlMul_exp1,0,0);
			//SetMultiFlashTimer();
		break;
	}
...
void SlaveMultiFlashPrepear(void)
{
	...
	{
		multiSet.finalExpTimes = MultiZero(canWireless.wlMul_exp1);
	}else
	{
...
//	if(!canWireless.wlMul_time)
//	{
//		canWireless.wlMul_time = MultiZero(canWireless.wlMul_exp1);
//	}

	//MulTimeRev();
	multiSet.finalExpTimes = canWireless.wlMul_time;//RfMulTimeCalc(20);
...
#ifdef FLASH_BASE_FIBER
			flaTime = CanOutputSwitch(canWireless.wlMul_exp1, 0);
			WlmFlaADSet(canWireless.wlMul_exp1, 0);
			TURN_ON_TRIGER(canWireless.wlMul_exp1>FLA_FBR_MAX?flaTime:OUTPUT_MAX);
#else
			flaTime = CanOutputSwitch(canWireless.wlMul_expose, 0);
			TURN_ON_TRIGER(flaTime);
#endif
			...
			if(1==radioStatue.cmdReady)
			{
				...
				}else if(RADIO_STATUE_MULTIFLASH==radioStatue.cmdSta)
				{
					if((WORKINGMODE_MULTI==mainPara.workMode)&&(g_chargeOk>0))
					{
						...
						TakeMultiFlash(&multiSet.finalExpTimes,canWireless.wlMul_exp1,multiSet.lowFrqCnt,0);
					}else
					{
						StopMultiFlash();
					}
...
void ComIdleFlash(void)
{
	...
	if((GetBatteryAdc()<LOW_BAT_LEVEL)||(BP_IN>0))
	{
		return;
	}	
