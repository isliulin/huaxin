...
*****************************************************/
	...
	//RCC->APB2ENR|=1<<4;//ÏÈÊ¹ÄÜÍâÉèPORTCÊ±ÖÓ//20150903
...
********************************************/
		...
		{
			if(EXTI_GetITStatus(EXTI_Line6) == SET)//                 £¯£¯ÅÐ¶ÏÊÇ·ñÊÇ£±£³½ÅµÄÖÐ¶Ï
		       {
#ifdef SLAVE_POWER_DOWN
				 ...
				 Ex_NVIC_Disable(6);						//2015/11/03 this must be closed, otherwise cause the error
		               EXTI_ClearITPendingBit(EXTI_Line6);//                £¯£¯Çå³ýÖÐ¶Ï±êÖ¾Î»£¡
		...
		case 0xa6:
			...
			if(menuPara.feb!=g_spiRcvData[4])
			{
				...
				if(g_stepLength>0)
				{
					...
					flaPara.ttl_FEB2 = H_COV_5(menuPara.feb);
				}else
				{
...
/***************************************************************************
		-3          -2          -1           0		1		2		3
		|___|___|___|___|___|___|___|___|___|___|___|___|				//²½³¤Îª0.5EV
