 ...
 *   Target : Texas Instruments CC2530EM
  ...
  #define BSP_MAX_MODULATION_MAGNITUDE ( BSP_TIMER_CLK_KHZ * 10 / 100 ) // ten percent
    ...
    #define BSP_TIMER_FREE_RUN_INIT( )\
              st( BF_SET( T1CTL, 0, 0, 2 );        /* Stop the timer. */\
                  ...
                  T1STAT = BF_GEN( 0, 4, 1 )       /* clear compare 4 interrupt */\
                           | BF_GEN( 0, 3, 1 )     /* clear compare 3 interrupt */\
                           | BF_GEN( 0, 2, 1 )     /* clear compare 2 interrupt */\
                           | BF_GEN( 0, 1, 1 )     /* clear compare 1 interrupt */\
                           | BF_GEN( 0, 0, 1 )     /* clear compare 0 interrupt */\
                           | BF_GEN( 0, 5, 1 );    /* clear overflow interrupt */\
                  T1CC0L = (BSP_TIMER_CLK_KHZ - 1) & 0xFF; /* initialize overflow values */\
                  T1CC0H = ( (BSP_TIMER_CLK_KHZ - 1) >> 8 ) & 0xFF;\
                  T1CCTL0 = T1CCTL1                /* for all five compare registers */\
                  = T1CCTL2 = T1CCTL3\
                  = T1CCTL4 = BF_GEN( 0, 7, 1 )    /* use capture mode not RF mode */\
                              | BF_GEN( 0, 6, 1 )  /* disable compare interrupts */\
                              | BF_GEN( 0, 3, 3 )  /* choose any output mode */\
                              | BF_GEN( 0, 2, 1 ); /* disable compare mode */\
                  T1CCTL0 = BF_GEN( 1, 6, 1 )      /* enable compare 0 interrupt */\
                            | BF_GEN( 1, 2, 1 );   /* set compare 0 mode */\
    ...
    #define BSP_TIMER_CHECK_OVERFLOW_FLAG( )      ( BF_GET( T1STAT, 0, 1 ) != 0 )
    #define BSP_TIMER_CLEAR_OVERFLOW_FLAG( )      ( T1STAT = ( T1STAT & BF_CLR( 0, 1 ) ) )
    ...
    #define BSP_TIMER_FREE_RUN_INIT( )\
              st( BF_SET( TxCTL, 0, 0, 4 );        /* Stop the timer. */\
                  ...
                  TxCCTL0                          /* for channel 0 compare registers */\
                          = BF_GEN( 1, 6, 1 )      /* enable channel 0 compare interrupts */\
                          | BF_GEN( 7, 3, 3 )      /* action on output when interrupt occurs */\
                          | BF_GEN( 1, 2, 1 );     /* enable channel 0 compare operation */\
                  TxCCTL1 = BF_GEN( 0, 6, 1 )      /* disable channel 1 compare interrupts */\
                          | BF_GEN( 7, 3, 3 )      /* not using channel 1 compare modes */\
                          | BF_GEN( 0, 2, 1 );     /* disable channel 1 compare operation */\
