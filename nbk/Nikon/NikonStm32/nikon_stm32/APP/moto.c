...
//char const focLenVal[11] =      {20,24,28,35,50,70,80,105,135,200};
//char const focLenScape[10] = {20,24,28,35,50,70,80,105,135,200};
//char const focLenScape[12] =  {20,24,28,35,50,70,85,105,120,135,180,200};
char const focLenScape[MOTO_MAX_INDEX] =  {20,24,28,35,50,70,85,105,120,135,180,200,250};
//char const focLenScape[MOTO_MAX_INDEX] =  {20,24,28,35,50,70,85,105,120,135,180,200,200};
//char const focLenScape[MOTO_MAX_INDEX] =  {20,24,28,35,50,180,85};
...
//u16 const focLenCoordinate[12] =  {20,108,182,398,614,856,910,980,1000,1020,1030,1040};
//u16 const focLenCoordinate[MOTO_MAX_INDEX] =  {10,20,30,108,182,398,614,856,910,980,1000,1020,1030,1040};
u16 const focLenCoordinate[MOTO_MAX_INDEX] =  {20,108,182,398,614,856,910,980,1000,1020,1030,1040,1050};
//u16 const focLenCoordinate[MOTO_MAX_INDEX] =  {20,108,182,398,614,856,910,980,1000,1020,1020,1040,1040};
//u16 const focLenCoordinate[MOTO_MAX_INDEX] =  {20,108,182,398,614,1020,910};

static u8 s_chargeCon = 0;
static u8 s_chargeLock=0;
u8 g_chargeShadow=0;
u16 g_flectDestRsv=0;
...
char const MotoDrv_Forward[] = {0x00,0x04,0x14,0x10};
char const MotoDrv_Backward[] = {0x10,0x14,0x04,0x00};
//char const focLenScape[11] = {24,28,35,50,70,85,105,120,135,180,200};
...
*/
		...
		}else
		{
			...
			if(errCnt>=MOTO_STEPS_TIMEOUT)
			{
				...
				zoomAdc = GetZoomAdc();
#ifdef MOTO_DEBUG_PRT
				DBG_PRINTF("Moto Error, zoomAdc %d\r\n",zoomAdc);
#endif
				SET_UPDATE_EVT(DSP_UPDATE_MODE);
				return;
			}else
			...
			if(!REF_BOARD_DETECT)
			{
				if(ref_flag>0)
				{
					...
					*motoSet.dest_location = g_flectDestRsv;
					motoSet.destRsv = *motoSet.dest_location;
					motoSet.dspLocation = g_flectDestRsv;
					//DisplayFocLen(*motoSet.mode,*motoSet.dest_location,DSP_NORMAL); 	
					SET_UPDATE_EVT(DSP_UPDATE_MODE);
					g_isReflectPulled = 0;
					//FLAOK_TOGGLE();
...
void MotoInvalidMove(u16 steps, u8 dir)
{
	u8 i = 0;
...
*/
//signed char GetMotoStdIndex(u8 dest)
signed char GetMotoStdIndex(u16 dest)
{
	u8 i=0;
	
	if(dest>=200)
	{
		return MOTO_MAX_INDEX-2;
	}else if(dest<=20)
	{
		return 0;
	}
