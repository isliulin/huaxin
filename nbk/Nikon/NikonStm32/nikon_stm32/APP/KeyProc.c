...
void KeyOk(void)
{
	...
	{
		case SETTING_PFNMENU:
			//if((g_snrIndex!=15)||(g_isReflectPulled>0))
			if((g_snrIndex!=SNR_WP)||(g_isReflectPulled>0))
			{
				g_SettingIndex = SETTING_PFNSUB;
				g_snrSubIndex = pSnrSet[g_snrIndex];
				/*
				SubSnrClear();
				//g_snrMenu[g_snrIndex].subMenu->subDsp(pSnrSet[g_snrIndex],pSnrSet[g_snrIndex],0x80|(1<<g_snrSubIndex));
				//g_snrMenu[g_snrIndex].subMenu->subDsp(pSnrSet[g_snrIndex],pSnrSet[g_snrIndex],DSP_REVERSE);
				DspSubMenu(pSnrSet[g_snrIndex],DSP_REVERSE);
				//BuildSnrMenu();	
				BuildSnrSubMenu();
				*/
				SET_UPDATE_EVT(DSP_UPDATE_MODE);
...
void EXTI1_IRQHandler(void)
{
	...
	EXTI_GetITStatus(EXTI_Line1);    //20170309 for debug
}
...
void HotProtectScreen(void)
{
	LCD_Clear(0);
	DisplayPicture(4,(172-40)/2,40,40,picTempStopFlash,DSP_NORMAL);
	DisplayPicture(3,(172-26-15),26,48,picTempWarn,DSP_NORMAL);
}
...
void DspUpdate(void)
{
	if(g_dspUpdateFlag&DSP_UPDATE_MODE)
	{
		...
		}else
		{
			...
			}else if(SETTING_PFNSUB == g_SettingIndex)
			{
				//g_SettingIndex = SETTING_PFNMENU;
				SubSnrClear();
				//g_snrMenu[g_snrIndex].subMenu->subDsp(pSnrSet[g_snrIndex],pSnrSet[g_snrIndex],0x80|(1<<g_snrSubIndex));
				...
				//BuildSnrMenu();	
				BuildSnrSubMenu();
			}else if(SETTING_MYMENU == g_SettingIndex)
			{
				//DspSnrMenu();
				DspMyMenuSelect();
			}else
