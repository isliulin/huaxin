...
void UserInit(void)
{
	...
	*g_cmdMode = CMD_MODE_LIGH_SLA;
	...
	if(mainPara.sumVeri!=sum)
	{
		...
		pSnrSet[SNR_CONTRAST] = 5;                   //2017/03/12
	}
...
{
	DBG_PRINTF("IWDG_Init enter\r\n");
	IWDG_WriteAccessCmd(IWDG_WriteAccess_Enable); //enable wdt write
	IWDG_SetPrescaler(prer); //set IWDT prescaler
	IWDG_SetReload(rlr); //set IWDT reloader
