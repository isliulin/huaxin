...
//CHECK OK
//091209
void Stm32_Clock_Init(u8 PLL)    // 内部时钟源
{
...
void SetMstPwrOnExti(u8 triMod)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE);//        ／／务必打开ＧＰＩＯＡ和ＡＦＩＯ时钟！！！

	GPIO_EXTILineConfig(GPIO_PortSourceGPIOA,GPIO_PinSource3);//                ／／配置Ａ口得１３脚为中断
       EXTI_ClearITPendingBit(EXTI_Line3);//                ／／清除中断，貌似不加也没得问题，但是保险一下！
	...
	//NVIC_InitStructure.NVIC_IRQChannel = EXTI3_IRQChannel;//                         ／／通道１３属于１５－１０之间，它们几个是共用的！
	NVIC_InitStructure.NVIC_IRQChannel = EXTI3_IRQn;//                         ／／通道１３属于１５－１０之间，它们几个是共用的！
...
void SleepWakeIntSet(void)
{
	...
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOA,GPIO_PinSource1);//                ／／配置Ａ口得１３脚为中断
       EXTI_ClearITPendingBit(EXTI_Line1);//                ／／清除中断，貌似不加也没得问题，但是保险一下！
	...
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_0);
	//NVIC_InitStructure.NVIC_IRQChannel = EXTI1_IRQChannel;//                         ／／通道１３属于１５－１０之间，它们几个是共用的！
	NVIC_InitStructure.NVIC_IRQChannel = EXTI1_IRQn;//                         ／／通道１３属于１５－１０之间，它们几个是共用的！
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;//                ／／占先优先级０
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;//                         　／／副优先级０
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;//
	NVIC_Init(&NVIC_InitStructure);
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;//                ／／占先优先级０
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;//                         　／／副优先级０
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;//
	NVIC_Init(&NVIC_InitStructure);
