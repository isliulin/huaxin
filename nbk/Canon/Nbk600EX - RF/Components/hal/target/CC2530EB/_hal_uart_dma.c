...
#define HAL_UART_DMA_FULL         (HAL_UART_DMA_RX_MAX - 16)//1//(HAL_UART_DMA_RX_MAX - 16)//changed by cbyi
#endif
...
void UartDMAIntRcv(void);
void UartDMAIntSend(void);
...
static void HalUARTPollDMA(void)
{
	...
	uint8 evt = 0;
/*************************************************************/
//cbyi for DMA transfer immediately
//	uint8  ch;
...
//cbyi for DMA transfer immediately
	...
	{
		if(evt!=HAL_UART_TX_EMPTY)
		//if(HAL_UART_RX_TIMEOUT==evt)
		{
			dmaCfg.uartCB(HAL_UART_DMA-1, evt);
		}
	}
