...
void *BtnGrModRes(void)
{
	...
	if (SYNC_MODE_FP == mainPara.synMode)
	{
		...
		{
			mainPara.synMode = SYNC_MODE_FRON;
			LcdClearFeild(LAYOUT_PARA_AREA0,LAYOUT_SYNMODE_LINE,FONT_SYN_WT,FONT_SYN_HT);
		}
	}
...
void DspLightSlavePara(char rvs)
{
	...
	switch(mainPara.workMode)
	{
		...
		case WORKINGMODE_MULTI:
			...
			DspSlavePara(WORKINGMODE_MULTI,canWireless.wlMul_expose,0,*wlSlave.s_array,rvs);
		...
		case WORKINGMODE_GR:
			...
			switch(*g_cmdMode)
			{
				...
				case CMD_MODE_RF_MST:
					...
					if (IsGrModExta())
					{
						AddButtons(&mstFlaSw,&ttlFeb,&mNull,&menu2,funcMenu.listCnt++);
					}else
					{
