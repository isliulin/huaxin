...
u8 RfSlaveMcodeInvert(u8 mCode,u8 hpMod,signed char* e1,signed char *e2)
{
	...
	for(index=0;index<sizeof(output_table);index++)
	{
		if(mCode==output_table[index])
		{
			//return index;
			break;
		}
	...
	if(index<sizeof(output_table))
	{
		//g_expTTLPara1 = index/3;
		//g_expTTLPara2 = (index+3)%3;
		index = 21 - index;
		*e1 = (index/3);
		if(e2!=NULL)
		{
...
void RfSlaveFlashCalc(signed char mod)
{
	...
	switch(mod)
	{
		...
		case WORKINGMODE_MULTI:
			...
			RfSlaveMulticodeInvert(g_rfSlaveFlashCode, 0, (signed char *)&canWireless.wlMul_expose, NULL);
			//*multiSet.expose<<=4;
			...
			g_expTTLPara1 = canWireless.wlMul_expose;
