...
MAC_INTERNAL_API void macMcuInit(void)
{
  ...
  /* start timer */
  MAC_RADIO_TIMER_WAKE_UP();				//启动时会经常停在此处，
  ...
  */
  ...
  /* put 16 random bits into the seed value */
  {
     ...
     *  random sequence won抰 be random.  There is an extremely small chance this seed could randomly
 ...
 *
 * @brief       For CC2530, T2 interrupt won抰 be generated when the current count is greater than
...
HAL_ISR_FUNCTION( macMcuRfIsr, RF_VECTOR )
{
  ...
  else if ((RFIRQF1 & IRQ_TXACKDONE) & rfim)			//SEND_OBEY_ZSTACK, 2014/11/5, I forget why delete here long time ago
  {
    /* disable interrupt - set up is for "one shot" operation */
    RFIRQM1 &= ~IM_TXACKDONE;
    macRxAckTxDoneCallback();
    //macExTxAckDoneCallback();
  }/*
  else if((RFIRQF1 & IRQ_TXDONE) & rfim)
  {
	RFIRQF1 &= ~IRQ_TXDONE;
	macExTxDoneCallback();
  }else if((RFIRQF1 & IRQ_TXACKDONE) & rfim)
  {
	RFIRQF1 &= ~IRQ_TXACKDONE;
	macExTxAckDoneCallback();
  }*/
