package gbpe.policeass.Plate.recognizer.ui;
...
import gbpe.policeass.Plate.EasyPR.PlateRecognizerImp;
import gbpe.policeass.Plate.recognizer.base.BaseActivity;
import gbpe.policeass.Plate.util.PlateFileUtil;
import gbpe.policeass.R;
import gbpe.policeass.utils.AppLog;

public class PlateRecognizeActivity extends BaseActivity implements SurfaceHolder.Callback {
    ...
    private static final String TAG = PlateRecognizeActivity.class.getSimpleName();
    ...
    private PlateRecognizerImp mPlateRecognizer;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_plate_recognize);

        ButterKnife.bind(this);

        mPlateRecognizer = new PlateRecognizerImp(this);
...
//                mCamera.setParameters(params);
    ...
    Camera.PictureCallback jpgPictureCallback = new Camera.PictureCallback() {
        ...
        public void onPictureTaken(byte[] data, Camera camera) {
            ...
            File pictureFile = PlateFileUtil.getOutputMediaFile(PlateFileUtil.FILE_TYPE_IMAGE);
            if (pictureFile == null) {
...
//                }
                // 最后通知图库更新
                PlateRecognizeActivity.this.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse("file://" + pictureFile.getAbsolutePath())));
                Toast.makeText(PlateRecognizeActivity.this, "图像已保存。", Toast.LENGTH_SHORT).show();
            } catch (FileNotFoundException e) {
...
//        this.setResult(RESULT_OK, intent);
    ...
    public void cropBitmapAndRecognize(Bitmap originalBitmap) {
        ...
        int rectWidth = originalBitmap.getWidth()/3;
        int rectHight = originalBitmap.getHeight()/6;
        ...
        location[0] = originalBitmap.getWidth()/3;
        location[1] = originalBitmap.getHeight()/2;
        Bitmap normalBitmap = Bitmap.createBitmap(originalBitmap, location[0], location[1], rectWidth, rectHight);

        // 保存图片并进行车牌识别
        File pictureFile = PlateFileUtil.getOutputMediaFile(PlateFileUtil.FILE_TYPE_PLATE);
        if (pictureFile == null) {
            Log.d(TAG, "Error creating media file, check storage permissions: ");
            return;
        }
