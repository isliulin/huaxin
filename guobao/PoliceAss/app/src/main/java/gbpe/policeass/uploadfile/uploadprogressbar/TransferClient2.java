package gbpe.policeass.uploadfile.uploadprogressbar;
...
class TransferClient2 {
   ...
   * @param filePath
   */
  public TransferClient2(String filePath){
...
//
   ...
   */
  public TransferClient2(ArrayList<String> fileList){
...
// 对代码进行适量封装
  public void service(){
      //ExecutorService executorService = Executors.newCachedThreadPool();//创建和上传的文件数一样多的线程，当文件较多时，会占满手机内存，适合少两文件，频繁上传。
      ExecutorService executorService = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());//根据系统资源设置线程次的大小

      for(int i=0;i<fileList.size();i++){
          String filePath = fileList.get(i);

          if (!Setup.isCancleUpload){//判断是否取消上传文件
      ...
      return new Runnable(){
          ...
          public void run() {
              ...
              if(createConnection()){         //判断是否连接成功
                  ...
                  try {
                      DataInputStream fis = new DataInputStream(new BufferedInputStream(new FileInputStream(filePath)));//从文件中读取数据
                      ...
                      if("noexsits".equals(marklength) || isFirstSendFileToServer ){//服务器端不存在该文件，故第一次上传该文件
                          ...
                          while((num=fis.read(buf))!=-1 ){
                              ...
                              if (Setup.isCancleUpload){//如果按取消则终止上传
                                  return;
                              }
                              dos.write(buf,0,num);
                              dos.flush();
                           ...
                           //   callBack.flieUpLoadProcess(Setup.currentProgress);//将数据发给fragment显示，更新UI的值
                          ...
                          while((serstart=raf.read(buf))!=-1){
                           ...
                           //   Setup.setCurrentProgress(serstart);
                            //  callBack.flieUpLoadProcess(Setup.currentProgress);
...
//android自定义view的学习
  ...
  public interface MyCallBack{//定义客户端回调函数，让fragment实现该回调接口（将当前的文件读取进度传给UI线程，更新UI的值即进度条的进度值）
      public void flieUpLoadProcess(long data);
  }
